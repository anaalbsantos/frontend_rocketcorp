...
import { FiPlusCircle, FiEdit2, FiX, FiTrash2 } from "react-icons/fi";
...
const Modal: React.FC<ModalProps> = ({ onClose, children, title }) => (
  <div
    ...
    role="dialog"
  >
    <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative max-h-[90vh]" style={{overflowY:"auto", scrollbarWidth:"thin", scrollbarColor:"#08605f #e2e8f0"}} onClick={(e) => e.stopPropagation()}>
      <button
        onClick={onClose}
...
const QuestionEditor: React.FC<QuestionEditorProps> = ({
  ...
  <div className="space-y-4 max-h-[60vh]" style={{overflowY:"auto",scrollbarWidth:"thin",scrollbarColor:"#08605f #e2e8f0"}}>
    {questions.map((q, i) => (
      <div key={q.id} className="border border-gray-300 rounded p-4 relative bg-white">
        ...
        <div className="mb-2">
          <label htmlFor={`pergunta-titulo-${q.id}`} className="block mb-1 text-sm font-medium text-gray-700">
            Pergunta {i + 1} - Título
          </label>
          <input
...
const PesquisaClima: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setPesquisas([
      { id: "1", titulo: "Clima - Julho/2025", descricao: "Pesquisa mensal sobre o clima organizacional da empresa.", status: "Em andamento", respostasAtuais: 45, respostasEsperadas: 60, data: "2025-07-30", perguntas: [] },
      { id: "2", titulo: "Onboarding Novos", descricao: "Feedback dos novos colaboradores sobre o processo de integração.", status: "Finalizada", respostasAtuais: 20, respostasEsperadas: 20, data: "2025-06-01", perguntas: [] },
    ]);
  }, []);

  useEffect(() => {
    if (pesquisas.length > 0) {
      localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify({ timestamp: Date.now(), data: pesquisas }));
    }
  }, [pesquisas]);
  ...
  const handleUpdateNovaPesquisa = useCallback((field: keyof Partial<Pesquisa>, value: any) => {
    setNovaPesquisa((prev) => ({ ...prev, [field]: value }));
  }, []);

  const handleUpdatePergunta = useCallback((id: string, field: keyof Pergunta, value: string) => {
    setNovaPesquisa((prev) => ({
      ...prev,
      perguntas: (prev.perguntas || []).map((q) => (q.id === id ? { ...q, [field]: value } : q)),
    }));
  }, []);
  ...
  const handleRemovePergunta = useCallback((id: string) => {
    setNovaPesquisa((prev) => ({
      ...prev,
      perguntas: (prev.perguntas || []).filter((q) => q.id !== id),
    }));
  }, []);
  ...
  const handleAddPergunta = useCallback(() => {
    setNovaPesquisa((prev) => ({
      ...prev,
      perguntas: [...(prev.perguntas || []), { id: String(Date.now()) + Math.random(), titulo: "" }],
    }));
  }, []);
  ...
  const irParaPerguntas = useCallback(() => {
    if (!novaPesquisa.titulo || !novaPesquisa.data || !novaPesquisa.respostasEsperadas) {
      alert("Preencha pelo menos título, data e preenchimentos esperados.");
      return;
    }
  ...
  const salvarNovaPesquisa = useCallback(() => {
    if (!novaPesquisa.titulo || !novaPesquisa.data || !novaPesquisa.respostasEsperadas) {
      alert("Título, data e preenchimentos esperados são obrigatórios.");
      return;
    }
  ...
  const salvarEdicao = useCallback(() => {
    if (!modalEdicao || !novaPesquisa.titulo || !novaPesquisa.data || !novaPesquisa.respostasEsperadas) {
      alert("Título, data e preenchimentos esperados são obrigatórios.");
      return;
    }
    setPesquisas((prev) =>
      prev.map((x) => (x.id === modalEdicao.id ? { ...x, ...novaPesquisa, respostasEsperadas: Number(novaPesquisa.respostasEsperadas) } as Pesquisa : x))
    );
    setModalEdicao(null);
  }, [modalEdicao, novaPesquisa]);
  ...
  return (
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm px-4 py-2 rounded"
              >
                <FiPlusCircle />
                Nova pesquisa
              </button>
              ...
              {pesquisasFiltradas.map((p) => (
                ...
                >
                  <div onClick={() => setModalDashboard(p)}>
                    ...
                    <p className="text-gray-600 mb-2 text-sm truncate max-h-16 overflow-hidden" title={p.descricao}>
                      {p.descricao}
                    </p>
                  ...
                  <div className="flex gap-2 absolute top-3 right-3">
                    <button
                      onClick={(e) => { e.stopPropagation(); abrirModalEdicao(p); }}
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); excluirPesquisa(p.id); }}
            ...
            <div className="p-4 border border-gray-200 rounded bg-gray-50 mb-4 mt-6">
              <h4 className="font-semibold text-gray-800 flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-gray-800">
                  {modalDashboard.respostasEsperadas > 0
                    ? `${Math.round((modalDashboard.respostasAtuais / modalDashboard.respostasEsperadas) * 100)}%`
                    : "0%"}
                </span>
              </h4>
              ...
              <div className="w-full bg-gray-200 h-2.5 rounded-full mt-2">
                <div
                  ...
                  style={{
                    width: `${modalDashboard.respostasEsperadas > 0 ? (modalDashboard.respostasAtuais / modalDashboard.respostasEsperadas) * 100 : 0}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <>
              ...
              <div className="mb-6 p-4 border border-gray-200 rounded bg-gray-50 min-h-[60px]">
                <div className="flex gap-4">
                  <div className="flex flex-col items-center bg-gray-50 p-6 rounded shadow w-1/3">
                    <span className="text-green-600 font-bold text-3xl">65%</span>
                    <span className="text-gray-600 text-sm">Sentimento Positivo</span>
                  </div>
                  <div className="flex flex-col items-center bg-gray-50 p-6 rounded shadow w-1/3">
                    <span className="text-yellow-600 font-bold text-3xl">20%</span>
                    <span className="text-gray-600 text-sm">Neutro</span>
                  </div>
                  <div className="flex flex-col items-center bg-gray-50 p-6 rounded shadow w-1/3">
                    <span className="text-red-600 font-bold text-3xl">15%</span>
                    <span className="text-gray-600 text-sm">Sentimento Negativo</span>
                  </div>
                </div>
    ...
    {modalCriarEtapa && (
      <Modal
        title={`Nova Pesquisa - Passo ${modalCriarEtapa === "info" ? "1" : "2"}`}
        onClose={() => setModalCriarEtapa(null)}
      >
          ...
          >
            <div>
              ...
              <input
                ...
                onChange={(e) => handleUpdateNovaPesquisa("titulo", e.target.value)}
                className="w-full border border-gray-300 rounded px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-600"
                required
                autoFocus
              />
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => handleUpdateNovaPesquisa("descricao", e.target.value)}
                className="w-full border border-gray-300 rounded px-3 py-2 resize-none bg-white focus:outline-none focus:ring-2 focus:ring-emerald-600"
              />
            </div>
            ...
            <div>
              <label
                className="block mb-1 text-sm font-medium text-gray-700"
                htmlFor="respostasEsperadas"
              >
                Preenchimentos Esperados
              </label>
              <input
                id="respostasEsperadas"
                type="number"
                value={novaPesquisa.respostasEsperadas || ""}
                onChange={(e) =>
                  handleUpdateNovaPesquisa("respostasEsperadas", Number(e.target.value))
                }
                className="w-full border border-gray-300 rounded px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-600"
                required
              />
            ...
            <div>
              ...
              <input
                id="data"
                type="date"
                value={novaPesquisa.data || ""}
                onChange={(e) => handleUpdateNovaPesquisa("data", e.target.value)}
                className="w-full border border-gray-300 rounded px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-600"
                required
              />
            </div>
            <div className="flex justify-between gap-2">
              <button
                type="button"
                className="w-1/2 py-2 rounded-lg bg-gray-500 text-white font-semibold"
                onClick={() => setModalCriarEtapa(null)}
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="w-1/2 py-2 rounded-lg bg-emerald-600 text-white font-semibold"
              >
                Próximo
              </button>
            </div>
        ...
        {modalCriarEtapa === "perguntas" && (
          <>
            <QuestionEditor
              questions={novaPesquisa.perguntas || []}
            ...
            <div className="flex gap-4 mt-6">
              <button
                type="button"
                className="w-1/2 py-2 rounded-lg bg-gray-500 text-white font-semibold"
                onClick={() => setModalCriarEtapa("info")}
              >
                Voltar
              </button>
              <button
      ...
      {modalEdicao && (
        <Modal title="Editar Pesquisa" onClose={() => setModalEdicao(null)}>
          <div className="space-y-4">
            ...
            />
            <div className="flex gap-4 mt-6">
              <button type="button" className="w-1/2 py-2 rounded-lg bg-gray-500 text-white font-semibold" onClick={() => setModalEdicao(null)}>
                Cancelar
              </button>
              <button type="button" className="w-1/2 py-2 rounded-lg bg-emerald-600 text-white font-semibold" onClick={salvarEdicao}>
                Salvar alterações
              </button>
            </div>
          </div>
