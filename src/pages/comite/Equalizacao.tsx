...
interface Usuario {
  id: string; name: string; role: string; positionId?: string; scorePerCycle: ScorePerCycle[];
}
interface CicloAtual { id: string }
interface APIResponse { usuarios: Usuario[]; cicloAtual?: CicloAtual }
interface Colaborador {
  id: string; nome: string; cargo: string; status: "Pendente" | "Finalizado";
  autoevaluationScore: number | null; managerEvaluationScore: number | null;
  evaluation360Score: number | null; summaryText: string; isEditable: boolean;
...
const EqualizacaoPage: React.FC = () => {
  ...
  const handleEditResult = async (id: string) => {
    ...
    try {
      ...
      setColaboradores((old) =>
        old.map((c) =>
          c.id === id ? { ...c, status: "Pendente", isEditable: true, notaFinal: null } : c
        )
      );
    } catch (error) {
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start md:items-center p-4 gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4">
                  ...
                  <span className={clsx("text-[10px] sm:text-[14px] font-bold px-2 py-1.5 rounded-lg leading-none", {
