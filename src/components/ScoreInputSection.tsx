...
const ScoreInputSection: React.FC<ScoreInputSectionProps> = ({
  autoevaluationScore, managerEvaluationScore, evaluation360Score,
  summaryText, isEditable, status, isVisible = true,
  onScoreChange, onJustificationChange, onDownload, onEdit, onConcluir
}) => {
  ...
  useEffect(() => { setSelectedScore(Math.round(autoevaluationScore)); }, [autoevaluationScore]);
  useEffect(() => {
    if (isVisible) {
      ...
      return (
        <Star
          ...
          className={clsx(
            ...
            { "cursor-pointer hover:fill-yellow-500 hover:text-yellow-500": isClickable, "cursor-not-allowed opacity-60": !isClickable }
          )}
          onClick={() => { if (isClickable) { setSelectedScore(i + 1); onScoreChange?.(i + 1); } }}
        />
      );
    });
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex gap-4 pt-4 border-b border-gray-200 pb-4 px-6">
        <div className="w-full flex flex-col gap-1">
          ...
          <Progress
            value={(autoevaluationScore / 5) * 100}
        ...
        <div className="w-full flex flex-col gap-1">
          <div className="flex justify-between items-center">
            <p className="text-text-muted font-bold text-xs">Avaliação Gestor</p>
      ...
      <div className="flex bg-[#F8F8F8] rounded-sm mt-4 mx-6 min-h-[150px]">
        <div className="border-l-4 border-[#08605F] rounded-sm self-stretch"></div>
          ...
          <div className="flex flex-col gap-4 px-6 mb-2">
              ...
              <>
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    ...
                    value={justification}
                    onChange={e => { setJustification(e.target.value); onJustificationChange?.(e.target.value); }}
                  />
                </div>
                <div className="flex gap-2 justify-end mb-2">
                  <button type="button" onClick={onConcluir} className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-green-600 text-white font-semibold hover:bg-green-700 transition-colors duration-200 text-sm">Concluir</button>
                </div>
              </>
            )}
            ...
            {status === "Finalizado" && (
              <div className="flex gap-2 justify-end mb-2">
                ...
                <button type="button" onClick={onEdit} className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-[#08605F] text-white font-semibold hover:bg-[#064a49] transition-colors duration-200 text-sm">
                  <Edit size={16} /> Editar
                </button>
              </div>
            )}
